...
`;return this.stop(),this.#i.write(c),this}};function ei(e){return new Qo(e)}var B3=(0,$.blue)((0,$.dim)("internal only"));function bt(e,t,r){console.log(_f[e]+t),typeof r?.exit<"u"&&process.exit(r.exit)}async function Tf(e,t,r){if(!zF){bt("wait",e);try{let o=await t();o&&console.log(o),bt("success",e);return}catch(o){return bt("error",e),r?.printError!==!1&&console.error(o),o}}let n=ei({spinner:"simpleDots",prefixText:_f.wait+e}).start();try{let o=await t();n.stop(),bt("success",e),o&&console.log(o)}catch(o){return n.stop(),bt("error",e),r?.printError!==!1&&console.error(o.message),o}}var _f={wait:`\u{1F550}${(0,$.blue)("wait")}  - `,info:`${process.env.GITHUB_ACTIONS==="true"?"\u2139\uFE0F":"\u2139\uFE0F "}${(0,$.cyan)("info")}  - `,success:`\u2705${(0,$.green)("ready")}  - `,warn:`${process.env.GITHUB_ACTIONS==="true"?"\u26A0\uFE0F":"\u26A0\uFE0F "}${(0,$.yellow)("warn")}  - `,error:`\u{1F4A5}${(0,$.red)("error")}  - `,event:`\u26A1\uFE0F${(0,$.magenta)("event")}  - `,paymentPrompt:`\u{1F4B0}${(0,$.yellowBright)("plan")}  - `},zF=!0;function VF(e,t,r){let n=e?"release":"debug",o=/#exportFunction\((\w+)\)/g;return{name:"swift-files",setup(i){i.onResolve({filter:/^swift:/},s=>({path:M.default.join(s.resolveDir,s.path.replace(/^swift:/,"")),namespace:"swift",sideEffects:!1})),i.onLoad({filter:/.*/,namespace:"swift"},async s=>{let a=s.path,u=M.default.basename(a),l=M.default.join("compiled_raycast_swift",u),c=[],f="",D="",d="Sources",p=".raycast-swift-build",g=M.default.join(a,p);if(!b.existsSync(M.default.join(a,"Package.swift")))return{errors:[{text:`failed to import Swift package from "${a}". Make sure you reference the folder that includes the 'Package.swift' file`}]};if(await Tf("compiling swift package",async()=>{let _=await Rf("/bin/sh",["-c","swift package dump-package"],a,g),j;try{j=JSON.parse(_)}catch(B){throw new Error(`failed to parse the swift package: ${B}`)}if(f=j.name,!f)throw new Error("failed to read the schema name from swift package");for(let B of j.targets||[])if(B.type==="executable"){if(D)throw new Error("failed to parse the swift package: expected one executable target but found more than one");D=B.name,B.path&&(d=B.path)}if(!D)throw new Error("failed to parse the swift package: expected one executable target but found none");let I=`xcodebuild build -skipMacroValidation -skipPackagePluginValidation -configuration ${n} -scheme ${f} -destination "generic/platform=macOS,name=Any Mac" -derivedDataPath ${p}`;await Rf("/bin/sh",["-c",I],a,g);let L=M.default.join(g,"Build/Products",n,D),Fe;try{if(Fe=b.statSync(L),!(Fe.mode&73))throw new Error("not executable")}catch{throw new Error(`failed to compile swift: Could not find the compiled asset.
